...
NETInt=$(for (( i=0; i<${#netname_array[@]}; i++ )); do
...
UDPPORTINFO=$(netstat -tuln | awk '/^udp/ {print "udp:"$4}'| awk -F ':' '{print $1" "$NF}'|sort -n  -k2| uniq | awk 'BEGIN {printf "%s", "UDP Ports: "} {printf "%s, ", $2} END {print ""}')
# delete the "," at the end of the string.
UDPPorts=${UDPPORTINFO%??}
...
TCPPORTINFO=$(netstat -tuln | awk '/^tcp/ {print "tcp:"$4}'| awk -F ':' '{print $1" "$NF}'|sort -n  -k2| uniq | awk 'BEGIN {printf "%s", "TCP Ports: "} {printf "%s, ", $2} END {print ""}')
# delete the "," at the end of the string.
TCPPorts=${TCPPORTINFO%??}
